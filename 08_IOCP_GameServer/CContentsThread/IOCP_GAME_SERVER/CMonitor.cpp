...
void CMonitor::UpdateServer() noexcept
{
	...
	for (int i = 0; i < ((CGameServer *)NET_SERVER::g_NetServer)->m_iEventCount; i++)
	{
		InterlockedExchange(&((CGameServer *)NET_SERVER::g_NetServer)->m_ArrTestEvent[i]->m_FPS, 0);
	}
...
void CMonitor::MonitoringConsole() noexcept
{
	...
	g_Logger->WriteLogConsole(LOG_LEVEL::SYSTEM, L"Monitor");

	for (int i = GAME_SETTING::WORK_COUNT - 20; i < ((CGameServer *)NET_SERVER::g_NetServer)->m_iEventCount; i++)
	{
		g_Logger->WriteLogConsole(LOG_LEVEL::SYSTEM, L"\tEvent%d FPS : %d", i, ((CGameServer *)NET_SERVER::g_NetServer)->m_ArrTestEvent[i]->m_FPS);
	}
