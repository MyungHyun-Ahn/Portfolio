...
private:
	...
	inline void CheckEnqueueEventQ() noexcept
	{
	...
	inline bool WorkStealing() noexcept
	{
		for (CContentsThread *thread : s_arrContentsThreads)
		{
			if (thread == this)
	...
	inline bool DelegateWork() noexcept
	{
	...
	inline void ProcessTimerEvent() noexcept
	{
		TimerEvent *pTimerEvent;
		LONG dTime;
		{
			...
			m_PrevSleepTime = dTime; // - 면 현재 수행 중
			if (dTime > 0)
			{
				Sleep(dTime);  
				return;
