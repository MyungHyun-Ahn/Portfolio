...
void CNetServer::SendPacket(const UINT64 sessionID, CSerializableBuffer<FALSE> *sBuffer) noexcept
{
	...
	LONG back = InterlockedIncrement(&pSession->m_iIOCountAndRelease);
	LONG logIndex = InterlockedIncrement(&pSession->logIndex);
	pSession->log[logIndex % MEM_LOG_MAX] = { 3, back };
	...
	back = InterlockedDecrement(&pSession->m_iIOCountAndRelease);
	logIndex = InterlockedIncrement(&pSession->logIndex);
	pSession->log[logIndex % MEM_LOG_MAX] = { 3, back };
	if (back == 0)
	{
		ReleaseSession(pSession);
...
BOOL CNetServer::Disconnect(const UINT64 sessionID) noexcept
{
	...
	LONG back = InterlockedIncrement(&pSession->m_iIOCountAndRelease);
	LONG logIndex = InterlockedIncrement(&pSession->logIndex);
	pSession->log[logIndex % MEM_LOG_MAX] = { 2, back };
	if ((pSession->m_iIOCountAndRelease & CNetSession::RELEASE_FLAG) == CNetSession::RELEASE_FLAG)
	{
		back = InterlockedDecrement(&pSession->m_iIOCountAndRelease);
		logIndex = InterlockedIncrement(&pSession->logIndex);
...
BOOL CNetServer::ReleaseSession(CNetSession *pSession) noexcept
{
	...
	LONG back = InterlockedCompareExchange(&pSession->m_iIOCountAndRelease, CNetSession::RELEASE_FLAG, 0);
	LONG logIndex = InterlockedIncrement(&pSession->logIndex);
	pSession->log[logIndex % MEM_LOG_MAX] = { 5, pSession->m_iIOCountAndRelease };
	if (back != 0)
	{
	...
	while (m_bIsWorkerRun)
	{
		...
		else
		{
			...
			case IOOperation::ACCEPTEX:
			{
				...
				LONG back = InterlockedIncrement(&pSession->m_iIOCountAndRelease);
				LONG logIndex = InterlockedIncrement(&pSession->logIndex);
				pSession->log[logIndex % MEM_LOG_MAX] = { 4, back };
