...
bool CNetSession::PostRecv() noexcept
{
	...
	LONG back = InterlockedIncrement(&m_iIOCountAndRelease);
	LONG index = InterlockedIncrement(&logIndex);
	log[index % MEM_LOG_MAX] = { 0, back };
	if ((m_iIOCountAndRelease & CNetSession::RELEASE_FLAG) == CNetSession::RELEASE_FLAG)
	{
		back = InterlockedDecrement(&m_iIOCountAndRelease);
		LONG index = InterlockedIncrement(&logIndex);
		log[index % MEM_LOG_MAX] = { 0, back };
		return FALSE;
	}
