...
void CProfileManager::DataOutToFile()
{
	...
	{
		fprintf_s(fp, "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
		fprintf_s(fp, "%-15s | %-100s \t| %-15s | %-15s | %-15s | %-15s | %-15s |\n", "ThreadId", "Name", "TotalTime(us)", "Average(us)", "Min(us)", "Max(us)", "Call");
		fprintf_s(fp, "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
		...
		for (int profileIdx = 0; profileIdx < pInfo->currentProfileSize; profileIdx++)
		{
			...
			double average = totalTime / (pProfile->iCall - 4);

			fprintf_s(fp, "%-15d | %-100s \t| %-15lld | %-15.06llf | %-15.06llf | %-15.06llf | %-15lld |\n"
				, pInfo->threadId
				, pProfile->szName
				, totalTime
				, average / m_lFreq.QuadPart * MICRO_SECOND
				, pProfile->iMinTime[0] / m_lFreq.QuadPart * MICRO_SECOND
				, pProfile->iMaxTime[0] / m_lFreq.QuadPart * MICRO_SECOND
				, pProfile->iCall - 4);
		}
